...
### NMF from scratch: use Lee - Euclidean distance https://proceedings.neurips.cc/paper_files/paper/2000/file/f9d1152547c0bde01830b7e8bd60024c-Paper.pdf
NMF_ELee_original<-function(V, k, M=NULL, itnum = 1000, eps=1e-9, epsDiff=1e-4){
  # V: input matrix
  # k: selected rank
  ...
  for(i in 1:itnum){
    ...
    e= sum((V1-W%*%H)^2)
    err=c(err,e)
    if (e < eps ||  abs(e-e1)/e1 < epsDiff) break()
    
    e1=e
    
    #if (!is.null(M)) V1= M*V+ (1-M) * V1
    
  }
  
  return(list(W = W, H = H, err = err))
}
