...
def load_mat_without_name(filepath):
    mat = scio.loadmat(filepath)
    drug_sim = mat["drug"].astype(np.float)
    disease_sim = mat["disease"].astype(np.float)
    #drug_name = mat["Wrname"].reshape(-1)
    ...
    interactions = mat["didr"]
    return drug_sim, disease_sim, drug_num,  disease_num, interactions
...
def data_preparation(args):
    ...
    assert args.dataset in ["Cdataset", "Fdataset", "Ldataset", "LRSSL", "HDVD", "LAGCN", "Ydataset", "oMatMechDB", "hsdnMechDB", "Fdataset_mul", "Cdataset_mul", "Ydataset_mul"]
    if args.dataset in ['Fdataset', 'Cdataset','Ydataset','Fdataset_mul', 'Cdataset_mul','Ydataset_mul']:
        drug_sim, disease_sim, drug_name, drug_num, disease_name, disease_num, interactions = load_mat(
                                                                                                path + args.dataset + ".mat")
    elif args.dataset in ['HDVD', 'LAGCN', 'LRSSL', 'oMatMechDB', 'hsdnMechDB']:
        drug_sim, disease_sim, drug_num, disease_num, interactions = load_mat_without_name(
                                                                        path + args.dataset + ".mat") 
        ...
        if type == "train":
            ...
            for i in range(len(train_mask)):
                for j in range(len(train_mask[0])):
                    disease_input.append(i)
                    drug_input.append(j)
                    indices.append((i, j))  # <--- Add this line
                    if train_mask[i][j]:
                        labels.append(disease_drug_Adj[i][j])
                    else:
                        labels.append(0)
            ...
            for i in range(num_batch):
                ...
                input_drug = drug_input[i * batch_size: (i + 1) * batch_size]
                label = labels[i * batch_size: (i + 1) * batch_size]
                batch_indices = indices[i * batch_size: (i + 1) * batch_size]  # <--- Add this line
                self.batch_data.append([input_disease, input_drug, label, batch_indices])  # <--- Add indices
...
#     filepath = "../dataset/Fdataset/Fdataset.mat"
#     drug_sim, disease_sim, drug_name, drug_num, disease_name, disease_num, interactions = load_mat(filepath)
#     print("drug_sim shape:", drug_sim.shape)
#     print("disease_sim shape:", disease_sim.shape)
#     print("drug_name:", drug_name[:5])
#     print("drug_num:", drug_num)
#     print("disease_name:", disease_name[:5])
