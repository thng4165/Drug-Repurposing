...
for (fold_index in 1: nFold){
  ...
  for (id in 1:N){
    ...
    B_random[is.na(fold)]= theta ## HDVD do not need
    ...
    nmfB <- NMF_ELee(V = B_random,  k = k_ran, itnum=200, eps=1e-9, epsDiff=1e-2, seed = myseed2) #standard
    B_update_random = nmfB$W %*% nmfB$H
    h0=B_update_random[is.na(fold)]
...
save(res_metric, file = "C:/Trang/KIProjects/ComprehensionDR/RerunMatlabCode/NMF_NMFPermutation/Result_1run/NMFPer/NMFPer_hsdn_res_metric.RData")
save(prediction_matrix, file = "C:/Trang/KIProjects/ComprehensionDR/RerunMatlabCode/NMF_NMFPermutation/Result_1run/NMFPer/NMFPer_hsdn_prediction.RData")
